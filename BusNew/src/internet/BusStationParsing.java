...
public class BusStationParsing {
	...
	public BusStationParsing(XmlPullParser parser, ArrayList<BusInfo> busInfoArray){
		parser.next(); // 문서의 시작은 넘어감
		int status = parser.getEventType(); // 현재 parser가 가르키는 곳의 상황을 가져옴.문서의
											// 시작이라던지, 태그의 시작점이라던지..
				...
				if (tag.equals("li")) {
					...
					if (count>0) {
						if (parser.getAttributeValue(0).equals("bsnm png")) {
							parser.next(); // <p class="bsnm png">정류소명 : <span
